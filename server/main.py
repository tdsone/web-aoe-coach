...
async def parse_replay(background_tasks: BackgroundTasks, file: UploadFile = File(...)):
    ...
    try:
        ...
        try:
            ...
            def flatten_players(data):
                for player in data:
                    if isinstance(player, dict):
                        for idx, team_player in enumerate(player.get("team", [])):
                            if isinstance(team_player, dict):
                                team_player_number = team_player.get("number")
                                team_player_index = data.index(team_player_number)
                                if team_player_index is not None:
                                    data[team_player_index] = team_player
            ...
            print(len(parsed_data.get("players", [])))
            print(parsed_data.get("teams", [])) 
