...
export function StatsChart({ statistics, currentTime, gameDuration, isLoading }: StatsChartProps) {
  ...
  // Generate X axis ticks (every 5 minutes)
  const xTicks = useMemo(() => {
    const ticks = []
    for (let t = 0; t <= gameDuration; t += 300) {
      ticks.push(t)
    }
    return ticks
  }, [gameDuration])
      ...
      <div className="flex-1 relative p-4">
        <svg
          ...
          onMouseMove={(e) => {
            ...
            const time = ((x - padding.left) / (800 - padding.left - padding.right)) * gameDuration
            ...
            chartData.players.forEach(player => {
              player.points.forEach(pt => {
                ...
                if (dist < minDist && dist < gameDuration * 0.05) {
                  minDist = dist
                  closest = {
                    x: toSvgX(pt.time, 800),
                    y: toSvgY(pt.value, 300),
                    time: pt.time,
                    value: pt.value,
                    player: player.name
                  }
        ...
        >
          ...
          <line
            x1={toSvgX(currentTime, 800)}
            y1={padding.top}
            x2={toSvgX(currentTime, 800)}
            y2={300 - padding.bottom}
            stroke="#fbbf24"
            strokeWidth={2}
            strokeOpacity={0.8}
          ...
          {chartData.players.map(player => {
            ...
            const pathD = player.points
              .map((pt, i) => `${i === 0 ? 'M' : 'L'} ${toSvgX(pt.time, 800)} ${toSvgY(pt.value, 300)}`)
              .join(' ')
            ...
            return (
              <g key={player.id}>
                ...
                {/* Area fill */}
                <path
                  d={`${pathD} L ${toSvgX(player.points[player.points.length - 1].time, 800)} ${300 - padding.bottom} L ${toSvgX(player.points[0].time, 800)} ${300 - padding.bottom} Z`}
                  fill={player.color}
                  fillOpacity={0.1}
                />
              </g>
